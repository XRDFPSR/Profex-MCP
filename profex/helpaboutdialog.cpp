...
{
    QString strSetInfo;
    strSetInfo += QString("<b>Executables</b><br>");
    strSetInfo += QString("bgmn: %1<br>").arg(settings->value("bgmnProject/bgmnExec", "not found").toString());
    strSetInfo += QString("makegeq: %1<br>").arg(settings->value("bgmnProject/makegeqExec", "not found").toString());
    strSetInfo += QString("geomet:  %1<br>").arg(settings->value("bgmnProject/geometExec", "not found").toString());
    strSetInfo += QString("teil:  %1<br>").arg(settings->value("bgmnProject/teilExec", "not found").toString());
    strSetInfo += QString("eflech: %1<br>").arg(settings->value("bgmnProject/eflechExec", "not found").toString());

    if (settings->value("config/useWine", false).toBool()) {
        strSetInfo += QString("wine: %1<br>").arg(settings->value("config/wineExec", "not found").toString());
    }

    strSetInfo += QString("<br><b>Databases</b><br>");
    strSetInfo += QString("Structures: %1").arg(settings->value("bgmnProject/structureDatabaseIndexed", "not found").toStringList().join("; "));
    strSetInfo += QString("%1<br>").arg(settings->value("bgmnProject/structureDatabaseNonIndexed", "not found").toStringList().join("; "));
    strSetInfo += QString("Devices: %1<br>").arg(settings->value("bgmnProject/deviceDatabase", "not found").toStringList().join("; "));
    strSetInfo += QString("Presets: %1<br><br>").arg(settings->value("bgmnProject/presetDirectory", "not found").toStringList().join("; "));

    strSetInfo += QString("<b>Log file</b><br>%1<br><br>").arg(logDest);
    strSetInfo += QString("<b>Reference <i>hkl</i> buffer file</b><br>%1<br><br>").arg(refStrManager->getHklBufferFileName());
    strSetInfo += QString("<b>Temporary file location</b><br>%1<br><br>").arg(settings->getTempLocation());
...
QString HelpAboutDialog::getAckText()
{
    ...
    strAck += "<b>QCustomPlot</b><br><a href=\"http://www.qcustomplot.com/\">http://www.qcustomplot.com/</a><br><br>";
    strAck += "<b>ALGLIB</b><br><a href=\"http://www.alglib.net/\">http://www.alglib.net/</a><br><br>";
