/***************************************************************************
                          nexusrawimport.cpp  -  description
                             -------------------
...
NexusRawImport::NexusRawImport(QObject *parent) :
    GenericImport(parent)
{
    extens << "hdf5";
    descr = QLatin1String("Anton Paar HDF5 Scan");
}

bool NexusRawImport::isSupported(const QByteArray &ba)
{
    return ba.left(5).toLower() == " hdf ";
}

int NexusRawImport::load(const QString &file, QVector<Scan> &scanHeap, bool minimal)
{
    Q_UNUSED(minimal);
    ...
    if (content.isEmpty()) {
        qDebug() << QString("NexusRawImport::load(): Empty file read from %1").arg(file);
        return -1;
    }
