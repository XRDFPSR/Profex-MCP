...
#ifndef POLYNOM5CURVE_H
#define POLYNOM5CURVE_H
...
class XRDIO_EXPORT Polynom4Curve : public GenericCurve
{
public:
    Polynom4Curve();
    ...
    inline CurveType type() const {return POLY4;}

    static inline QString descriptionStatic() {return "Polynomial 4";}
    static inline QString equationTextStatic() {return QString("f(x) = a + bx + cx^2 + dx^3 + ex^4");}
    ...
    inline QStringList lowerLimits(double, double, double) const {return QStringList() << "-inf"  << "-inf"  << "-inf"  << "-inf"  << "-inf" ;}
