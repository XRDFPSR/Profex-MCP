/***************************************************************************
                          lorentziansplitcurve.h  -  description
                             -------------------
...
#ifndef LORENTZIANSPLITCURVE_H
#define LORENTZIANSPLITCURVE_H
...
class XRDIO_EXPORT LorentzianSplitCurve : public GenericCurve
{
public:
    LorentzianSplitCurve();

    ~LorentzianSplitCurve() {}

    inline CurveType type() const {return LORENTZIANSPLIT;}

    static inline QString descriptionStatic() {return "Split Lorentzian";}
    static inline QString equationTextStatic() {return QString("x <= a: f(x) = c / (1 + ((x-a) / b1)^2)\nx >  a: f(x) = c / (1 + ((x-a) / b2)^2)");}
