...
#ifndef PEARSONCURVE_H
#define PEARSONCURVE_H
...
class XRDIO_EXPORT PearsonCurve : public GenericCurve
{
public:
    PearsonCurve();
    ...
    inline CurveType type() const {return PEARSONVII;}

    static inline QString descriptionStatic() {return "Pearson-VII";}
    static inline QString equationTextStatic() {return QString("f(x) = c / (1.0 + ((x - a) / b)^2 * ((2.0, 1.0 / d) - 1.0))^d");}
    ...
    inline QList<double> defaultValues() const {return QList<double>() << -1.0 << 0.001 << 1.0 << 2.0;}
    inline QStringList lowerLimits(double mi, double, double ss) const {return QStringList() << QString::number(mi, 'f', 5) << QString::number(ss, 'f', 5) << "0.00000" << "1.00000";}
