...
AsciiXyExport::AsciiXyExport(QObject *parent) :
    GenericExport(parent)
{
    uId = "ASCII_XY";
}

int AsciiXyExport::save(const QString &file, const Scan &scan, const QMap<QString, QVariant> &flags)
{
...
int AsciiXyExport::save(const QString &file, const QVector<Scan> &scanHeap, const QMap<QString, QVariant> &flags)
{
    bool fix = flags.value("fixBgmnZero", false).toBool();
    QString sep(flags.value("fieldSeparator", " ").toString());
...
QString AsciiXyExport::getData(const Scan &scan, const QString &sep, bool fix)
{
    QString data;
...
QString AsciiXyExport::getData(const QVector<Scan> &scanHeap, const QString &sep, bool fix)
{
    QString data;
    QTextStream stream(&data);
    stream.setRealNumberNotation(QTextStream::FixedNotation);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int s = 0; s < (int)vecs.size(); s += 2) {
            ...
            if ((i >= dang.size()) || (i >= dint.size())) continue;
...
double AsciiXyExport::fixZero(double d)
{
